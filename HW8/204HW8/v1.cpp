...
void cashier(int min, int max, int treshold, int cashierID)
{
	...
	{
		myMutex.lock();
		if ((queueSize != 0 && cashierID == 1) || (cashierID == 2 && queueSize >= treshold)) 
		{  
...
int main()
{
	...
	thread thr1(&cashier, min_checkout_time, max_checkout_time, cashier2_threshold, 1);
	thread thr2(&cashier, min_checkout_time, max_checkout_time, cashier2_threshold, 2);
